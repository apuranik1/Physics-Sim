...
public class PhysicsManager {
				...
				for (Object3D other : engine.intersects(obj.getBoundingBox()))
					if (other != obj) {
						...
						if (otherSpec.isCollidable() && collides) {
							...
							// handle both special collisions, since they will
							// be moved apart
							if (specialCollides)
	...
	private void handleCollision(Object3D obj0, Object3D obj1) {
		...
		double m0 = obj0.getSpec().getMass(), m1 = obj1.getSpec().getMass();
		...
		obj0.setVelocity(obj0.getVelocity().add(
				problemVeloc.multiply(-1.3 * refMass / m0)));
		obj1.setVelocity(obj1.getVelocity().add(
				problemVeloc.multiply(1.3 * refMass / m1)));
	}
	...
	private static void translateAway(Object3D obj0, Object3D obj1,
			double dist, Vector3D axis, double obj0MoveProp) {
		// TODO: test to hopefully fix collision handling
		BoundingBox bb0 = obj0.getBoundingBox(), bb1 = obj1.getBoundingBox();
		// double dist = bb0.distance(bb1, axis);
		Vector3D newPos0 = bb0.getLocation().add(
				axis.multiply(dist * 1.0000001 * obj0MoveProp));
		Vector3D newPos1 = bb1.getLocation().add(
				axis.multiply(-dist * 1.0000001 * (1 - obj0MoveProp)));
